...
<script type="text/ecmascript-6">
	...
	export default {
		...
		methods: {
			...
			setToken(token, expiresInMinutes) {
			  const expiryTime = new Date().getTime() + expiresInMinutes * 60 * 1000; // 设置过期时间为当前时间 + 指定分钟数
			...
			submitForm(formName) {
				this.$refs[formName].validate(valid => {
					if (valid) {
						...
						requests.post("/login", this.ruleForm).then(response => {
							if (response.data.code != 200) {
								...
								this.setToken(response.data.data,30)
